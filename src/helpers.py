...
def plot_qubo_diagonal(plot_deets, grid_details, Q_obj,
                       points_of_interest=None, existing_chargers=None, gas_stations=None,
                       show_data_points=True, show_qubo_values=True,
                       show_grid_ids=True, poi_style='density'):
    """
    Visualize the diagonal QUBO values Q_obj[(i, i)] on the grid.

    Negative values are green, zero is white, and positive values are yellow.
    This makes the per-cell "solo" attractiveness/penalty pattern easy to
    compare against the cell-weight heatmap.
    """
    x_min = plot_deets['x_min']
    x_max = plot_deets['x_max']
    y_min = plot_deets['y_min']
    ...
    diag_matrix = np.zeros((num_rows, num_cols))
    for gid, info in grid_details.items():
        diag_matrix[info['row'], info['col']] = Q_obj.get((gid, gid), 0.0)

    max_abs = float(np.max(np.abs(diag_matrix))) if diag_matrix.size else 1.0
    if max_abs == 0.0:
        max_abs = 1.0

    qubo_cmap = mcolors.LinearSegmentedColormap.from_list(
        'qubo_diag_green_white_yellow',
        ['#238b45', '#ffffff', '#ffd54f'],
        N=256,
    )
    qubo_norm = mcolors.TwoSlopeNorm(vmin=-max_abs, vcenter=0.0, vmax=max_abs)
    ...
    show_values_here = show_qubo_values and num_rows * num_cols <= 900
    if show_grid_ids or show_values_here:
        for gid, info in grid_details.items():
            cx = (info['x_start'] + info['x_end']) / 2
            cy = (info['y_start'] + info['y_end']) / 2
            q_val = Q_obj.get((gid, gid), 0.0)

            label_parts = []
            if show_grid_ids:
                label_parts.append(str(gid))
            if show_values_here:
                label_parts.append(f'{q_val:+.2f}')
            ...
            if label_parts:
                ax.annotate(
                    ...
                    ha='center',
                    va='center',
                    fontsize=max(4, min(8, 200 // max(num_rows, num_cols))),
                    color='black',
                    alpha=0.8,
                )
    ...
    ax.set_xlabel('X Coordinate')
    ax.set_ylabel('Y Coordinate')
    ax.set_title(f'QUBO Diagonal Heatmap — {num_rows}x{num_cols} Grid ({num_rows * num_cols} cells)')
    ax.legend(loc='upper right')
    ax.set_aspect('equal')
    plt.tight_layout()
    plt.show()
