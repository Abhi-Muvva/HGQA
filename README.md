...
<!-- | Tier | K after pruning | Execution mode | 
|------|----------------|---------------|
| **Small** | K ≤ ~20 | Brute-force (C(K, m) enumeration) or exact statevector QAOA | Ground truth — no approximation |
| **Medium** | K ≤ ~50–80 | MPS-based QAOA (Qiskit Aer) | PoC-valid simulation, low-entanglement circuits |
| **Large** | K > 80 | MPS-based QAOA with reduced bond dimension | Approximate; used to demonstrate seeding benefit in principle | -->
...
<!-- **On MPS simulation and PoC validity:**
MPS (Matrix Product State) simulation is a classical method. For low-depth QAOA (p ≤ 3), circuits build limited entanglement and MPS can represent the statevector exactly at sufficient bond dimension. The valid PoC argument: if MPS-seeded GA outperforms random-seeded GA, this demonstrates the *seeding benefit* as a structural property — one that would persist with real quantum hardware. MPS does **not** demonstrate quantum speedup and this claim is never made. -->
...
surviving_cells, report = prune_cells(
    ...
    N,           # total grid cells before pruning
    m,           # chargers to place
    num_cols,    # grid columns (for Chebyshev distance)
    R4,          # H4 spacing radius (used in Tier 1 safety check)
    tier3_keep,  # cells to keep per micro-cluster (default: 3)
    verbose,     # print per-tier report
)
```

`report` contains per-tier statistics: cells pruned, cells surviving, greedy reference score (Tier 2), cluster sizes (Tier 3).

After pruning, the Q_obj is remapped to new 0-indexed qubit variables for QAOA:

```python
Q_pruned, cell_to_qubit, qubit_to_cell = remap_qubo_for_qaoa(Q_obj, surviving_cells)
```

QAOA results (in qubit indices) are translated back to original cell IDs before being passed to the GA:

```python
translated = translate_results(qaoa_results, qubit_to_cell)
``` -->
...
Before trusting pruner output on any new problem instance:
1. **Tier 3 brute-force check (q=4, N=16, m=2):** Enumerate all C(16,2)=120 solutions, find the true optimal, run full pruner pipeline, verify the optimal solution's cells survive Tier 3.
2. **MPS agreement check:** On small pruned instances (K ≤ 20), verify statevector QAOA and MPS QAOA return the same top-3 solutions. If they diverge, increase bond dimension.
3. **Score monotonicity:** The greedy reference score from Tier 2 should always be ≤ the final GA solution score. If GA somehow beats it, the reference was too tight (impossible by construction — this is a sanity check). -->
...
<!-- #### Mode 3 — Noisy Fake Backend (`run_qaoa_noisy`)
- Uses `SamplerV2` with a Qiskit IBM Runtime fake backend (e.g., `FakeGuadalupeV2`)
- Transpiles the parametrized circuit once, binds parameters per COBYLA evaluation
- Final sampling with higher shot count for solution extraction
- **Use when:** Testing realistic hardware noise effects. Secondary to Mode 2 for PoC purposes. -->
...
| **Hybrid (QAOA/MPS + GA)** | Proposed method: QAOA or MPS-seeded initial population |
| **Only brute-force (small instances)** | Ground truth for correctness verification: C(K,m) enumeration |

Note: a dedicated "Only QAOA" comparison (without GA refinement) may be included for completeness, matching the structure in the reference paper, but it is not the primary comparison of interest.

### Validation Sequence

Before running full experiments, three validation checks must pass:

| Check | What | Expected |
|-------|------|----------|
| **Tier 3 brute-force** | Run full pruner on q=4 (N=16, m=2), verify optimal solution survives | Optimal cell IDs in surviving set |
| **MPS agreement** | Run statevector QAOA and MPS QAOA on same small pruned instance | Top-3 solutions identical or near-identical |
| **Score decomposition** | Evaluate known good vs known bad solution against Q_obj | Score gap ~10–15 units, correct sign |

### Metrics
- Primary QUBO fitness score (minimize)
- Convergence speed (generations to reach near-optimal)
- Solution stability (variance across 5+ runs)
- Paper's scoring metric (sum of min distances from each POI to nearest charger) — for benchmarking against reference paper results
...
- Bar charts comparing scores across methods and datasets (matching Figures 4 and 5 in the reference paper)
- Pruner report: N → K reduction per instance, per-tier breakdown -->
